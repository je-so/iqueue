...
int main(void)
{
   ...
   printf("iqueue1_t: %d*1000000 send/recv time in ms: %ld (%ld nr_of_msg/msec)\n", 1, msec, 1*1000000/msec);
   ...
   for (int nrthread = 1; nrthread <= MAXTHREAD; nrthread *= 2) {
      ...
      printf("iqueue_t: %d*1000000 send/recv time in ms: %ld (%ld nr_of_msg/msec)\n", nrthread, msec, nrthread*1000000/msec);
   }
